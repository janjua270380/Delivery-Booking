...
import { User, Mail, Building, Calendar, Trash2, Edit, Save, X, AlertTriangle, Shield, RefreshCw, CheckCircle, Search, UserPlus, Phone, Lock, Users, Eye, EyeOff, Sliders } from 'lucide-react';
...
export function AdminUserControls({ onBack }: AdminUserControlsProps) {
  ...
  const filteredCustomers = customers.filter(customer => {
    ...
    if (activeTab === 'all') {
      ...
      return matchesSearch && customer.isAdmin;
    } else if (activeTab === 'workers') {
      return matchesSearch && customer.isWorker;
                  ...
                  <div className="bg-gray-50 p-4 rounded-md space-y-3">
                    <div className="flex items-center">
                      <input
                        ...
                        checked={editCustomer.workerPermissions?.viewCustomers || false}
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        checked={editCustomer.workerPermissions?.manageBookings || false}
                        onChange={(e) => setEditCustomer({
                          ...editCustomer,
                          workerPermissions: {
                            ...editCustomer.workerPermissions!,
                    ...
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                        id="editViewPricing"
                        checked={editCustomer.workerPermissions?.viewPricing || false}
                        onChange={(e) => setEditCustomer({
                          ...editCustomer,
                          workerPermissions: {
                            ...editCustomer.workerPermissions!,
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        checked={editCustomer.workerPermissions?.managePricing || false}
                        onChange={(e) => setEditCustomer({
                          ...editCustomer,
                          workerPermissions: {
                            ...editCustomer.workerPermissions!,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-700">
                Note: Bookings associated with this user will be preserved, but will be marked as belonging to a deleted account.
              </p>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`${customer.isAdmin ? 'text-gray-400' : 'text-blue-600 hover:text-blue-900'}`}
                        title={customer.isAdmin ? "View admin account" : "Edit user"}
                      >
                        {customer.isAdmin ? <Eye className="w-4 h-4" /> : <Edit className="w-4 h-4" />}
                      </button>
                      <button
                        onClick={() => setShowConfirmDelete(customer.id)}
                        className={`${customer.isAdmin ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:text-red-900'}`}
                        disabled={customer.isAdmin}
                        title={customer.isAdmin ? "Admin accounts cannot be deleted" : "Delete user"}
                      >
